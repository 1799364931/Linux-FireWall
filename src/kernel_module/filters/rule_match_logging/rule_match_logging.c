...
static inline void log_rule_match_tcp_packet(uint32_t rule_id,
    ...
    // printk(KERN_INFO
    //        "%s [RULE_ID:%u] %s TCP %s.%u > %s.%u Flags[%s] seq %u ack %u win
    //        "
    //        "%u length %u\n",
    //        timestamp, rule_id, action, src_ip, src_port, dst_ip, dst_port,
    //        flags_str, ntohl(tcph->seq), ntohl(tcph->ack_seq),
    ...
    notify_user_event(reply_msg, REPLY_MSG_SIZE, user_portid,
...
static inline void log_rule_match_udp_packet(uint32_t rule_id,
    ...
    sprintf(reply_msg, "%s [RULE_ID:%u] %s UDP %s.%u > %s.%u length %u\n",
            timestamp, rule_id, action, src_ip, src_port, dst_ip, dst_port,
            ntohs(udph->len));
...
static inline void log_rule_match_icmp_packet(uint32_t rule_id,
    ...
    // 输出日志
    // printk(KERN_INFO
    //        "%s [RULE_ID:%u] %s ICMP %s > %s type=%s code=%u id=%u seq=%u "
    //        "length %u\n",
    //        timestamp, rule_id, action, src_ip, dst_ip, icmp_type_str,
    //        icmph->code, ntohs(icmph->un.echo.id),
    //        ntohs(icmph->un.echo.sequence), payload_len);
...
void log_rule_match(uint32_t rule_id,
    ...
    switch (iph->protocol) {
        ...
        default: {
            ...
            //        timestamp, rule_id, action, iph->protocol, src_ip,
            //        dst_ip);
            char* reply_msg = kmalloc(REPLY_MSG_SIZE, GFP_KERNEL);

            sprintf(reply_msg, "%s [RULE_ID:%u] %s Protocol %u %s > %s\n",
                    timestamp, rule_id, action, iph->protocol, src_ip, dst_ip);
